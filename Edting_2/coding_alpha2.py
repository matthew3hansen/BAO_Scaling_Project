...
def run(alpha=1.):
	...
	def quad_coeff_alpha_(helper2): #this is a
		temp = 0.0
		product = product_()
		precision_matrix = np.linalg.inv(helper2.covariance_matrix)
	...
	def linear_coeff_alpha_(helper2): #This is b
		temp = 0.0
		product = product_()
		precision_matrix = np.linalg.inv(helper2.covariance_matrix)
		for l in range(len(radial_bin_l_)):
	...
	def const_coeff_alpha_(helper2): #This is c
		temp = 0.0
		product = product_()
		precision_matrix = np.linalg.inv(helper2.covariance_matrix)
		for l in range(len(radial_bin_l_)):
	...
	def get_terms(helper2):
		xi_IRrs_alpha = np.zeros(len(helper2.r))

		precision = np.linalg.inv(helper2.covariance_matrix)

		for i in range(len(helper2.r)):
			xi_IRrs_alpha[i] = np.sum(1 / (2 * math.pi**2) * helper2.ks**2 * special.spherical_jn(0, alpha * helper2.ks * helper2.r_bins[i]) * np.exp(-helper2.ks**2) * helper2.P_IRres * np.gradient(helper2.ks))
		
		constant_term = 0
		linear_term = 0
		quadratic_term = 0
		third_term = 0
		for l in range(len(helper2.r)):
			for m in range(len(helper2.r)):
				constant_term += precision[l][m] * (- p1 * data_list[l] * p1 * helper2.xi_IRrs_prime[m] - p1 * data_list[m] * p1 * helper2.xi_IRrs_prime[l] \
						+ p1 * helper2.xi_IRrs_prime[l] * p1 * helper2.xi_IRrs[m] + p1 * helper2.xi_IRrs_prime[m] * p1 * helper2.xi_IRrs[l])

				linear_term += precision[l][m] * (- p1 * data_list[l] * p1 * helper2.xi_IRrs_prime2[m] - p1 * data_list[m] * p1 * helper2.xi_IRrs_prime2[l] \
								+ 2 * p1 * helper2.xi_IRrs_prime[l] * p1 * helper2.xi_IRrs_prime[m] + p1 * helper2.xi_IRrs_prime2[l] * p1 * helper2.xi_IRrs[m]\
								+ p1 * helper2.xi_IRrs_prime2[m] * p1 * helper2.xi_IRrs[l])

				quadratic_term += precision[l][m] * (0.5 * p1 * helper2.xi_IRrs_prime[l] * p1 * helper2.xi_IRrs_prime2[m] \
								+ p1 * helper2.xi_IRrs_prime2[l] * p1 * helper2.xi_IRrs_prime[m] + 0.5 * p1 * helper2.xi_IRrs_prime[m] * p1 * helper2.xi_IRrs_prime2[l] \
								+ p1 * helper2.xi_IRrs_prime2[m] * p1 * helper2.xi_IRrs_prime[l]) 

				third_term += precision[l][m] * (0.5 * helper2.xi_IRrs_prime2[l] * helper2.xi_IRrs_prime2[m] + 0.5 * helper2.xi_IRrs_prime2[l] * helper2.xi_IRrs_prime2[m])

		return (constant_term, linear_term, quadratic_term, third_term)

	def check_multiplication():
		print(np.shape(radial_bin_l_))
		print(p1[0])
		print('p1: ', p1[0])
		print('xi: ', xi_tensor[0])
		print('[xi_IRrs[i], xi_IRrs_prime[i], xi_IRrs_prime2[i]]', xi_IRrs[0], ' ', xi_IRrs_prime[0], ' ', xi_IRrs_prime2[0])
		print(np.shape(xi_tensor))
		p = np.array([p1[0]])
		p = p.transpose()
		product = np.matmul(p, np.array(xi_tensor))
	...
	def delta_alpha_(helper2):
		...
		'''
		const_coeff_alpha, linear_coeff_alpha, quad_coeff_alpha = const_coeff_alpha_(helper2), linear_coeff_alpha_(helper2), quad_coeff_alpha_(helper2) #get_terms(helper2)
		print('Testing:')
		print('const: ', const_coeff_alpha)
		print('lin: ', linear_coeff_alpha)
		print('quad: ', quad_coeff_alpha)
	...
	return delta_alpha_(helper_object)

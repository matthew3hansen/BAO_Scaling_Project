...
def marginalize_delta_alpha():
    ...
    b_da = 2 * b_a #(0.5 * np.multiply(precision, (temp + temp1 + temp2))).sum()
    
    #temp = np.multiply.outer(xi_IRrs, xi_IRrs_prime)
    #temp1 = temp.transpose()
    c_da = b_a#(0.5 * np.multiply(precision, (temp + temp1))).sum()
    ...
    #temp1 = temp.transpose()
    b_db = 2 * a_b #(0.5 * np.multiply(precision, (temp + temp1))).sum()
    
    #temp = np.multiply.outer(data_list, xi_IRrs_prime)
    #temp1 = temp.transpose()
    c_db = b_b#(0.5 * np.multiply(precision, (temp + temp1))).sum()

...
class Info:
    ...
    def calc_covariance_matrix(self):
        self.covariance_matrix = np.loadtxt('covariance_matrix_new.txt', usecols=range(30)) / 3.5
        
        '''
        #Old method for Covariance Matrix
        ...
        np.savetxt('covariance_matrix_old.txt', self.covariance_matrix_old)
        ...
        for i in range(len(self.r)):
            ...
            for j in range(len(self.r)):
                ...
                # use b1 ^2 * Pklin in the covariance matrix
                j0_return = 2./(self.effective_volume * self.number_density * np.pi**2.) * j0j0.rotation_method_bessel_j0j0(self.ks, self.b1 ** 2 * self.pk_lin_z0 * self.growth **2., R1, R2)
                # this one may need to be corrected to include P2, P4, if we are fitting xi0 (or are we fitting xi(r))?
                j0_return_pk_sq = 1./(self.effective_volume * np.pi**2.) * j0j0.rotation_method_bessel_j0j0(self.ks, (self.b1 ** 2 * self.pk_lin_z0 * self.growth **2.)**2., R1, R2)
                #print('j0:', time.time() - start_time)
  
                if i == j:
                    xi_IRrs_alpha = np.sum(1 / (2 * math.pi**2) * self.ks[:,np.newaxis]**2 * special.spherical_jn(0, self.alpha * self.ks[:,np.newaxis] * r_bins_1) \
  						* np.exp(-self.ks[:,np.newaxis]**2) * self.P_IRres[:, np.newaxis] * np.gradient(self.ks)[:,np.newaxis], axis=0)
                    #print(time.time() - start_time)
                    dirac_cf_diag = xi_IRrs_alpha / delta_r
                    dirac_cf_matrix = np.diag(dirac_cf_diag)
                    dirac_cf_matrix *= 1 / (self.effective_volume * self.number_density**2)
                    ...
                    self.covariance_matrix[i][j] = (4 * math.pi)**2. / (4/3 * math.pi * (self.r_bins[i+1]**3 - self.r_bins[i]**3))**2. \
                                                    * np.sum(R1 **2 * R2 **2 * (j0_return + j0_return_pk_sq + dirac_cf_matrix + dirac_matrix) * delta_r * delta_r)
                self.covariance_matrix[i][j] = (4 * math.pi)**2. / (4/3 * math.pi * (self.r_bins[i+1]**3 - self.r_bins[i]**3) * (self.r_bins[j+1]**3 - self.r_bins[j]**3)) \
                                                * np.sum(R1 **2 * R2 **2 * (j0_return + j0_return_pk_sq) * delta_r * delta_r)
        #print('i!=j:', time.time() - start_time)								
        np.savetxt('covariance_matrix_new.txt', self.covariance_matrix)
        '''
    ...
    def calc_CF(self):
        ...
        for i in range(len(self.r)):
            self.xi_gg_data[i] = np.sum(1 / (2 * math.pi**2) * self.ks**2 * special.spherical_jn(0, self.alpha * self.ks * self.r_bins[i]) * np.exp(-self.ks**2) * self.P_gg * np.gradient(self.ks))

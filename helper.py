...
class Info:
	...
	def calc_CF(self):
		...
		np.random.multivariate_normal(self.xi_gg, self.covariance_matrix)
	...
	def templates(self):
		...
		self.xi_d1d2 = np.zeros(len(self.r))
		self.xi_d2d2 = np.zeros(len(self.r))
		self.xi_d1s2 = np.zeros(len(self.r))
		self.xi_d2s2 = np.zeros(len(self.r))
		self.xi_s2s2 = np.zeros(len(self.r))
		self.xi_d1p3 = np.zeros(len(self.r))
		for i in range(len(self.r)):
			self.xi_IRrs[i] = np.sum(1 / (2 * math.pi**2) * self.ks**2 * special.spherical_jn(0, self.alpha * self.ks * self.r_bins[i]) * np.exp(-self.ks**2) * self.P_IRres * np.gradient(self.ks))
			self.xi_d1d2[i] = np.sum(1 / (2 * math.pi**2) * self.ks**2 * special.spherical_jn(0, self.alpha * self.ks * self.r_bins[i]) * np.exp(-self.ks**2) * self.Pd1d2 * np.gradient(self.ks))
			self.xi_d2d2[i] = np.sum(1 / (2 * math.pi**2) * self.ks**2 * special.spherical_jn(0, self.alpha * self.ks * self.r_bins[i]) * np.exp(-self.ks**2) * (self.Pd2d2 - 2.*self.s4) * np.gradient(self.ks))
			self.xi_d1s2[i] = np.sum(1 / (2 * math.pi**2) * self.ks**2 * special.spherical_jn(0, self.alpha * self.ks * self.r_bins[i]) * np.exp(-self.ks**2) * self.Pd1s2 * np.gradient(self.ks))
			self.xi_d2s2[i] = np.sum(1 / (2 * math.pi**2) * self.ks**2 * special.spherical_jn(0, self.alpha * self.ks * self.r_bins[i]) * np.exp(-self.ks**2) * (self.Pd2s2 - (4./3.)*self.s4) * np.gradient(self.ks))
			self.xi_s2s2[i] = np.sum(1 / (2 * math.pi**2) * self.ks**2 * special.spherical_jn(0, self.alpha * self.ks * self.r_bins[i]) * np.exp(-self.ks**2) * (self.Ps2s2 - (8./9.)*self.s4) * np.gradient(self.ks))
			self.xi_d1p3[i] = np.sum(1 / (2 * math.pi**2) * self.ks**2 * special.spherical_jn(0, self.alpha * self.ks * self.r_bins[i]) * np.exp(-self.ks**2) * self.Pd1p3 * np.gradient(self.ks))
		
		return self.xi_IRrs, self.xi_d1d2, self.xi_d2d2, self.xi_d1s2, self.xi_d2s2, self.xi_s2s2, self.xi_d1p3

	def templates_deriv(self):
		self.xi_IRrs_prime = np.zeros(len(self.r))
		self.xi_d1d2_prime = np.zeros(len(self.r))
		self.xi_d2d2_prime = np.zeros(len(self.r))
		self.xi_d1s2_prime = np.zeros(len(self.r))
		self.xi_d2s2_prime = np.zeros(len(self.r))
		self.xi_s2s2_prime = np.zeros(len(self.r))
		self.xi_d1p3_prime = np.zeros(len(self.r))
		for i in range(len(self.r)):
			self.xi_IRrs_prime[i] = -self.r_bins[i] * np.sum(1 / (2 * math.pi**2) * self.ks**3 * special.spherical_jn(1, self.alpha * self.ks * self.r_bins[i]) * np.exp(-self.ks**2) * self.P_IRres * np.gradient(self.ks))
			self.xi_d1d2_prime[i] = -self.r_bins[i] * np.sum(1 / (2 * math.pi**2) * self.ks**3 * special.spherical_jn(1, self.alpha * self.ks * self.r_bins[i]) * np.exp(-self.ks**2) * self.Pd1d2 * np.gradient(self.ks))
			self.xi_d2d2_prime[i] = -self.r_bins[i] * np.sum(1 / (2 * math.pi**2) * self.ks**3 * special.spherical_jn(1, self.alpha * self.ks * self.r_bins[i]) * np.exp(-self.ks**2) * (self.Pd2d2 - 2.*self.s4) * np.gradient(self.ks))
			self.xi_d1s2_prime[i] = -self.r_bins[i] * np.sum(1 / (2 * math.pi**2) * self.ks**3 * special.spherical_jn(1, self.alpha * self.ks * self.r_bins[i]) * np.exp(-self.ks**2) * self.Pd1s2 * np.gradient(self.ks))
			self.xi_d2s2_prime[i] = -self.r_bins[i] * np.sum(1 / (2 * math.pi**2) * self.ks**3 * special.spherical_jn(1, self.alpha * self.ks * self.r_bins[i]) * np.exp(-self.ks**2) * (self.Pd2s2 - (4./3.)*self.s4) * np.gradient(self.ks))
			self.xi_s2s2_prime[i] = -self.r_bins[i] * np.sum(1 / (2 * math.pi**2) * self.ks**3 * special.spherical_jn(1, self.alpha * self.ks * self.r_bins[i]) * np.exp(-self.ks**2) * (self.Ps2s2 - (8./9.)*self.s4) * np.gradient(self.ks))
			self.xi_d1p3_prime[i] = -self.r_bins[i] * np.sum(1 / (2 * math.pi**2) * self.ks**3 * special.spherical_jn(1, self.alpha * self.ks * self.r_bins[i]) * np.exp(-self.ks**2) * self.Pd1p3 * np.gradient(self.ks))
		
		return self.xi_IRrs_prime, self.xi_d1d2_prime, self.xi_d2d2_prime, self.xi_d1s2_prime, self.xi_d2s2_prime, self.xi_s2s2_prime, self.xi_d1p3_prime

	def templates_deriv2(self):
		self.xi_IRrs_prime2 = np.zeros(len(self.r))
		self.xi_d1d2_prime2 = np.zeros(len(self.r))
		self.xi_d2d2_prime2 = np.zeros(len(self.r))
		self.xi_d1s2_prime2 = np.zeros(len(self.r))
		self.xi_d2s2_prime2 = np.zeros(len(self.r))
		self.xi_s2s2_prime2 = np.zeros(len(self.r))
		self.xi_d1p3_prime2 = np.zeros(len(self.r))
		for i in range(len(self.r)):
			self.xi_IRrs_prime2[i] = self.r_bins[i]**2 * np.sum(1 / (2 * math.pi**2) * self.ks**4 * (special.spherical_jn(2, self.alpha * self.ks * self.r_bins[i]) - (1 / (self.ks * self.r_bins[i])) * special.spherical_jn(1, self.alpha * self.ks * self.r_bins[i])) * np.exp(-self.ks**2) * self.P_IRres * np.gradient(self.ks))
			self.xi_d1d2_prime2[i] = self.r_bins[i]**2 * np.sum(1 / (2 * math.pi**2) * self.ks**4 * (special.spherical_jn(2, self.alpha * self.ks * self.r_bins[i]) - (1 / (self.ks * self.r_bins[i])) * special.spherical_jn(1, self.alpha * self.ks * self.r_bins[i])) * np.exp(-self.ks**2) * self.Pd1d2 * np.gradient(self.ks))
			self.xi_d2d2_prime2[i] = self.r_bins[i]**2 * np.sum(1 / (2 * math.pi**2) * self.ks**4 * (special.spherical_jn(2, self.alpha * self.ks * self.r_bins[i]) - (1 / (self.ks * self.r_bins[i])) * special.spherical_jn(1, self.alpha * self.ks * self.r_bins[i])) * np.exp(-self.ks**2) * (self.Pd2d2 - 2.*self.s4) * np.gradient(self.ks))
			self.xi_d1s2_prime2[i] = self.r_bins[i]**2 * np.sum(1 / (2 * math.pi**2) * self.ks**4 * (special.spherical_jn(2, self.alpha * self.ks * self.r_bins[i]) - (1 / (self.ks * self.r_bins[i])) * special.spherical_jn(1, self.alpha * self.ks * self.r_bins[i])) * np.exp(-self.ks**2) * self.Pd1s2 * np.gradient(self.ks))
			self.xi_d2s2_prime2[i] = self.r_bins[i]**2 * np.sum(1 / (2 * math.pi**2) * self.ks**4 * (special.spherical_jn(2, self.alpha * self.ks * self.r_bins[i]) - (1 / (self.ks * self.r_bins[i])) * special.spherical_jn(1, self.alpha * self.ks * self.r_bins[i])) * np.exp(-self.ks**2) * (self.Pd2s2 - (4./3.)*self.s4) * np.gradient(self.ks))
			self.xi_s2s2_prime2[i] = self.r_bins[i]**2 * np.sum(1 / (2 * math.pi**2) * self.ks**4 * (special.spherical_jn(2, self.alpha * self.ks * self.r_bins[i]) - (1 / (self.ks * self.r_bins[i])) * special.spherical_jn(1, self.alpha * self.ks * self.r_bins[i])) * np.exp(-self.ks**2) * (self.Ps2s2 - (8./9.)*self.s4) * np.gradient(self.ks))
			self.xi_d1p3_prime2[i] = self.r_bins[i]**2 * np.sum(1 / (2 * math.pi**2) * self.ks**4 * (special.spherical_jn(2, self.alpha * self.ks * self.r_bins[i]) - (1 / (self.ks * self.r_bins[i])) * special.spherical_jn(1, self.alpha * self.ks * self.r_bins[i])) * np.exp(-self.ks**2) * self.Pd1p3 * np.gradient(self.ks))

		return self.xi_IRrs_prime2, self.xi_d1d2_prime2, self.xi_d2d2_prime2, self.xi_d1s2_prime2, self.xi_d2s2_prime2, self.xi_s2s2_prime2, self.xi_d1p3_prime2

	def get_biases(self):
		return [(self.b1 * self.b1), (self.b1 * self.b2), (self.b2 * self.b2), (self.b1 * self.bs), (self.b2 * self.bs), (self.bs * self.bs), (self.b1 * self.b3nl)]
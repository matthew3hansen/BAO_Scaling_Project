...
def run(alpha=1.):
	...
	def check_multiplication():
		print('p1: ', p1[radial_bin_l_[0]])
		print('xi: ', xi_tensor[radial_bin_l_[0]])
		p = np.array([p1[radial_bin_l_[0]]])
		p = p.transpose()
		print(np.matmul(p, np.array(xi_tensor[radial_bin_l_])).transpose())
	...
	def product_l_(p1, radial_bin_l):
		p = np.array([p1])
		p = p.transpose()
		return np.matmul(p, np.array(xi_tensor[radial_bin_l])).transpose()

	def product_m_(p1, radial_bin_m):
		p = np.array([p1])
		p = p.transpose()
		return np.matmul(p, np.array(xi_tensor[radial_bin_m])).transpose()
